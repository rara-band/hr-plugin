# Changelog

이 플러그인은 [유의적 버전](https://semver.org/lang/ko/)을 따른다 — `vMAJOR.MINOR.PATCH`.
`1.0.0` 이전(`0.y.z`)은 공개 전이라 언제든 바뀔 수 있다. 버전의 단일 소스는 `.claude-plugin/plugin.json`.

**버전 올리는 규칙** (플러그인 자신의 *변경 등급*과 맞춤):

| 올림 | = 변경 등급 | 언제 |
|---|---|---|
| **MAJOR** | 큰 변경 | 동작/계약이 깨짐 — 훅 이벤트·worklog 형식·출력 형식이 기존과 안 맞게 바뀜 |
| **MINOR** | 중간 변경 | 호환되는 기능 추가 — 새 발령 종류·새 신호·새 스킬 |
| **PATCH** | 작은 변경 | 문구·버그 수정, 동작은 그대로 |

릴리스할 때 git 태그를 같이 단다: `git tag v0.1.0`.

---

## [Unreleased]

## [0.4.0] — 2026-06-27
결과·품질 — 월급루팡(빈손·재작업)과 독(쓰였는데 나쁨)을 함께 본다.

### Added
- **결과 캡처(P1)** — 훅이 `result`(ok/empty/error)·`out_len`을 worklog에 적는다(`tool_result`에서 방어적으로). 분석가가 *반복된 빈손/재작업*을 "월급루팡"(불렸는데 헛돎)으로 → 자르기 후보. `out_len` 작다고 단정 안 함(§5).
- **리뷰 기반 점검** — `/hr-review <결과물 리뷰>`: HR이 목표 대비 점검 포인트 + 근거 기록을 묶어 제시 → 사장 판정을 받아 기여 역 추적 → *개선 가능한 실수 vs 도움 안 되는 역* 분류. 품질은 외부 리뷰로 받는다(§5).
- **거수기 아님** — 사장이 불가능·근거 부족·측정 못 하는 기준·직무 분리 위반을 시키면 거절하고 이유를 댄다.

### Changed
- 판정 축 "업무량" → "기여"(출근 + 결과).

## [0.3.0] — 2026-06-27
지시 모드 + 로스터 현황 스킬. HR이 pull(스스로 훑기)에 더해 push(요청자가 콕 집어 진단)도 한다.

### Added
- **지시 진단** — `/hr-review <자연어 우려>`(예: "이 역 이상해", "X 자르려는데 맞아?")로 그 대상에 집중. 가설을 *확인 또는 반박*한다(집중은 시선이지 편향이 아님). 출력 맨 앞에 가설 판정 + 근거.
- **`/hr-roster`** — 로스터 현황표만(역·교대 수·최근성·안 불린 역). 판정·권고 없이 사실만. 사실 보기와 판정을 분리.

## [0.2.0] — 2026-06-26
worklog 보존/정리.

### Added
- **worklog 정리** — 오래된 교대만 덜어낸다: 최근 90일 + 안전 상한 2만 줄 (`hooks/scripts/prune-worklog.sh`), 매 SessionStart에 자동. 판단(리뷰)과 분리 — 나이 기준이라 되돌아보기를 안 깬다.

### Changed
- DESIGN.md ③에 보존/정리 정책 명문화 (보존 90일 ⊇ 되돌아보기 기간).

## [0.1.0] — 2026-06-26
첫 버전. **"현재 에이전트 배치가 방향에 얼마나 핏한가"** 하나만 판단하는 검토 루프.

### Added
- **껍데기** — `plugin.json`·`marketplace.json`, AGENTS.md doctrine, HR 위키, 다국어 README.
- **근태 훅** — `PostToolUse(Task)`마다 한 교대를 `worklog.jsonl`에 적재 (`hooks/scripts/record-shift.sh`). 받아 적기만, 작업 흐름 안 막음.
- **`hr-analyst`** — 배치 핏 2-box(방향 적합 × 업무량) 판정 + "남아도나" 3단계 + 가드(직무 분리·되돌아보기 기간·측정 정직성).
- **`/hr-review`** — 방향 묻고 → worklog+명단 모아 분석가에 넘기고 → 확정 권고 + 개편안 → 승인 게이트.
- **DESIGN.md** — v1 동작 설계 (목표 → 맥락 → 수단 → 기준 → 출력). 훅 페이로드는 로컬 실측으로 확정.

### Notes
- 범위 밖(나중): 결과 캡처·합치기·채용 빈틈 분석·구조 판정·Codex 배선·시계열·품질(eval)·비용.
